//!GENERATED by tatara-rust-derive::KindRoundTripSpec.
//!Do not hand-edit; regenerate from the (defkind-round-trip …) source.
use proc_macro::TokenStream;
use quote::quote;
use syn::{parse_macro_input, Data, DataEnum, DeriveInput, Fields, LitStr};
use syn::LitInt;
#[proc_macro_derive(KindByte, attributes(kind))]
pub fn derive_kind_byte(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let self_name = &input.ident;
    let de = match &input.data {
        Data::Enum(DataEnum { variants, .. }) => variants,
        _ => {
            return syn::Error::new_spanned(self_name, "KindByte requires an enum")
                .to_compile_error()
                .into();
        }
    };
    let mut as_str_arms = Vec::new();
    let mut from_str_arms = Vec::new();
    let mut as_byte_arms = Vec::new();
    let mut from_byte_arms = Vec::new();
    let mut errors = Vec::new();
    for v in de.iter() {
        let variant_name = &v.ident;
        if !matches!(v.fields, Fields::Unit) {
            errors
                .push(
                    syn::Error::new_spanned(
                            variant_name,
                            "KindByte requires unit (fieldless) variants",
                        )
                        .to_compile_error(),
                );
            continue;
        }
        let mut name = variant_name.to_string();
        let mut aliases: Vec<String> = Vec::new();
        let mut byte: ::core::option::Option<u8> = ::core::option::Option::None;
        for attr in &v.attrs {
            if !attr.path().is_ident("kind") {
                continue;
            }
            let r = attr
                .parse_nested_meta(|meta| {
                    if meta.path.is_ident("name") {
                        let s: LitStr = meta.value()?.parse()?;
                        name = s.value();
                    } else if meta.path.is_ident("alias") {
                        let s: LitStr = meta.value()?.parse()?;
                        aliases.push(s.value());
                    } else if meta.path.is_ident("byte") {
                        let n: LitInt = meta.value()?.parse()?;
                        byte = ::core::option::Option::Some(n.base10_parse()?);
                    } else {
                        return Err(
                            meta.error("unknown `kind` key (expected name/alias/byte)"),
                        );
                    }
                    Ok(())
                });
            if let Err(e) = r {
                errors.push(e.to_compile_error());
            }
        }
        as_str_arms
            .push(
                quote! {
                    Self::# variant_name => # name,
                },
            );
        from_str_arms
            .push(
                quote! {
                    # name => ::core::option::Option::Some(Self::# variant_name),
                },
            );
        for a in &aliases {
            from_str_arms
                .push(
                    quote! {
                        # a => ::core::option::Option::Some(Self::# variant_name),
                    },
                );
        }
        match byte {
            ::core::option::Option::Some(b) => {
                as_byte_arms
                    .push(
                        quote! {
                            Self::# variant_name => # b,
                        },
                    );
                from_byte_arms
                    .push(
                        quote! {
                            # b => ::core::option::Option::Some(Self::# variant_name),
                        },
                    );
            }
            ::core::option::Option::None => {
                errors
                    .push(
                        syn::Error::new_spanned(
                                variant_name,
                                "KindByte (with_byte) requires every variant carry #[kind(byte = N)]",
                            )
                            .to_compile_error(),
                    );
            }
        }
    }
    if !errors.is_empty() {
        let mut ts = proc_macro2::TokenStream::new();
        for e in errors {
            ts.extend(e);
        }
        return ts.into();
    }
    let expanded = quote! {
        impl # self_name { #[doc =
        "Wire string for this variant (paired inverse of `from_str_kind`)."] pub fn
        as_str(& self) -> &'static str { match self { # (# as_str_arms) * } } #[doc =
        "Parse a variant from its wire string (paired inverse of `as_str`)."] pub fn
        from_str_kind(s : & str) -> ::core::option::Option < Self > { match s { # (#
        from_str_arms) * _ => ::core::option::Option::None, } } #[doc =
        "Wire byte for this variant (paired inverse of `from_byte`)."] pub fn as_byte(&
        self) -> u8 { match self { # (# as_byte_arms) * } } #[doc =
        "Parse a variant from its wire byte (paired inverse of `as_byte`)."] pub fn
        from_byte(b : u8) -> ::core::option::Option < Self > { match b { # (#
        from_byte_arms) * _ => ::core::option::Option::None, } } }
    };
    TokenStream::from(expanded)
}
